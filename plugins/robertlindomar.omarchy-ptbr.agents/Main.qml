...
Item {
  ...
  Process {
    ...
    onExited: function(exitCode) {
      if (exitCode !== 0) {
        if (root.syncConfigured()) root.syncStatusText = "Falha ao criar pasta de sincronização de uso"
        root.finishSyncRun()
        return
      }
  ...
  Process {
    ...
    onRunningChanged: root.updateSyncRunning()
    onExited: function(exitCode) {
      if (exitCode !== 0 && root.syncConfigured()) root.syncStatusText = "Falha ao ler pasta de sincronização de uso"
      root.finishSyncRun()
    }
  ...
  function friendlyModelName(id) {
    ...
    return words.length > 0 ? words.join(" ") : "Desconhecido"
  }
}

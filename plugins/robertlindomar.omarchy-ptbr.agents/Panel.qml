...
Panel {
  ...
  function windowTitle(label) {
    var text = String(label || "").toLowerCase()
    if (text.indexOf("month") >= 0) return "Mensal"
    if (windowIsLong(text)) return "Semanal"
    if (text.indexOf("session") >= 0 || windowSpanMs(label) > 0) return "Sessão"
    var plain = String(label || "").replace(/\s*\(.*\)\s*/, "").trim()
    return plain === "" ? "Limite" : plain
  }
  ...
  function balanceDetailText(b) {
    ...
    var text = formatMoney(b.spent, b.currency) + " gastos de " + formatMoney(b.funded, b.currency) + " financiados"
  ...
  function dayName(date) {
    ...
    return ["Dom", "Seg", "Ter", "Qua", "Qui", "Sex", "Sáb"][parsed.getDay()]
  }
  ...
  function dayTooltip(day, today) {
    ...
    if (today && provider && provider.hasPromptStats !== false)
      text += " · " + Number(provider.todayPrompts || 0) + " prompts · "
        + Number(provider.todaySessions || 0) + " sessões"
    return text
  }
  ...
  function modelTooltip(row) {
    ...
    return "Entrada " + usage.formatTokenCount(row.input)
      + " · saída " + usage.formatTokenCount(row.output)
      + " · cache leitura " + usage.formatTokenCount(row.cacheRead)
      + " · cache gravação " + usage.formatTokenCount(row.cacheWrite)
  }
  ...
  function footerText() {
    ...
    if (provider && provider.syncEnabled && provider.syncDeviceCount > 0)
      return "Mesclado de " + provider.syncDeviceCount + " dispositivo" + (provider.syncDeviceCount === 1 ? "" : "s")
    return ""
  }
      ...
      Flickable {
        ...
        Column {
          ...
          Text {
            ...
            text: "Nenhuma assinatura de codificação com IA encontrada.\nOs agentes aparecem aqui depois que você os utilizar."
            color: root.dim
            font.family: root.fontFamily
  ...
  component LimitRow: Column {
    ...
    Text {
      ...
      text: {
        ...
        return remainingMs > 0 ? "Reinicia em " + root.formatDuration(remainingMs) : ""
      }
      color: root.dim
      font.family: root.fontFamily
      font.pixelSize: Style.font.caption

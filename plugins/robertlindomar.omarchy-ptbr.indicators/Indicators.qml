...
BarWidget {
  ...
  readonly property var defaultPinnedIndicatorIds: [ "StayAwake", "Dnd", "Reminder" ]
  readonly property var pinnedIndicatorIds: pinnedIdsFromSettings(settings)
  readonly property var pinnedIndicatorEntries: filterIndicatorEntries(true)
  readonly property var hoverIndicatorEntries: filterIndicatorEntries(false)
  ...
  // Host proxy: indicadores fixos tratam inativos como sempre revelados (opacity/concealed).
  QtObject {
    id: pinnedRevealHost
    property bool revealInactiveIndicators: true
    function setIndicatorAreaHovered(hovered) { root.setIndicatorAreaHovered(hovered) }
    function setIndicatorItemHovered(hovered) { root.setIndicatorItemHovered(hovered) }
  }
  ...
  function pinnedIdsFromSettings(settings) {
    var pinned = setting("pinned", defaultPinnedIndicatorIds)
    if (!pinned || typeof pinned.length !== "number" || pinned.length === 0)
      return defaultPinnedIndicatorIds
    ...
    return result.length ? result : defaultPinnedIndicatorIds
  }

  function isPinnedIndicator(id) {
    return pinnedIndicatorIds.indexOf(id) !== -1
  }

  function filterIndicatorEntries(pinned) {
    var result = []
    for (var i = 0; i < indicatorEntries.length; i++) {
      var entry = indicatorEntries[i]
      var id = entryId(entry)
      if (pinned ? isPinnedIndicator(id) : !isPinnedIndicator(id))
        result.push(entry)
    }
    return result
  }
  ...
  onPinnedIndicatorIdsChanged: syncActiveIndicatorOrder()
  ...
  Row {
    ...
    Item {
      ...
      implicitWidth: pinnedInactiveHorizontalBlock.implicitWidth
        + (root.revealInactiveIndicators ? hoverInactiveHorizontalBlock.implicitWidth : 0)
      implicitHeight: Math.max(
        pinnedInactiveHorizontalBlock.implicitHeight,
        hoverInactiveHorizontalBlock.implicitHeight,
        root.barSize
      )
      width: implicitWidth
      ...
      Row {
        ...
        IndicatorBlock {
          id: pinnedInactiveHorizontalBlock
          indicatorsModule: root
          indicatorEntries: root.pinnedIndicatorEntries
          indicatorBlock: "inactive"
          horizontal: true
          reportActiveState: !root.vertical
          forceInactiveVisible: true
        }

        IndicatorBlock {
          id: hoverInactiveHorizontalBlock
          indicatorsModule: root
          indicatorEntries: root.hoverIndicatorEntries
          indicatorBlock: "inactive"
          horizontal: true
          reportActiveState: !root.vertical
          forceInactiveVisible: false
        }
      }
  ...
  Column {
    ...
    Item {
      ...
      implicitWidth: Math.max(
        pinnedInactiveVerticalBlock.implicitWidth,
        hoverInactiveVerticalBlock.implicitWidth,
        root.barSize
      )
      implicitHeight: pinnedInactiveVerticalBlock.implicitHeight
        + (root.revealInactiveIndicators ? hoverInactiveVerticalBlock.implicitHeight : 0)
      width: implicitWidth
      height: implicitHeight
      clip: true
      ...
      Column {
        ...
        IndicatorBlock {
          id: pinnedInactiveVerticalBlock
          indicatorsModule: root
          indicatorEntries: root.pinnedIndicatorEntries
          indicatorBlock: "inactive"
          horizontal: false
  ...
  component IndicatorBlock: Item {
    ...
    property bool horizontal: true
    property bool reportActiveState: false
    property bool forceInactiveVisible: false
      ...
      Row {
        ...
        Repeater {
          ...
          IndicatorLoader {
            ...
            forceInactiveVisible: indicatorBlockRoot.forceInactiveVisible
          }
        }
      }
    }
  ...
  component IndicatorLoader: Item {
    ...
    onForceInactiveVisibleChanged: injectProps()
    ...
    function injectProps() {
      ...
      if ("indicatorHost" in target) {
        target.indicatorHost = (indicatorBlock === "inactive" && forceInactiveVisible)
          ? pinnedRevealHost
          : root
      }

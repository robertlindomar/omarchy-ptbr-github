...
// Qt.formatDate(…, locale) não funciona de forma confiável no Quickshell —
// o terceiro argumento pode ser ignorado e textos como o hero somem.
var MONTH_NAMES = [
...
function monthYearLabel(year, month) {
  ...
  return (MONTH_NAMES[m] + " de " + year).toUpperCase()
}

function weekdayLongName(weekday) {
  var index = Number(weekday)
  if (!isFinite(index)) return ""
  return WEEKDAY_LONG[((index % 7) + 7) % 7]
}

// Abreviações no estilo da barra (Ter, Set) — Qt.formatDateTime ignora locale no shell.
var WEEKDAY_SHORT = ["Dom", "Seg", "Ter", "Qua", "Qui", "Sex", "Sáb"]
var MONTH_SHORT = ["Jan", "Fev", "Mar", "Abr", "Mai", "Jun", "Jul", "Ago", "Set", "Out", "Nov", "Dez"]

function formatClockDateTime(date, format) {
  if (!date || isNaN(date.getTime())) return ""
  ...
  var ap = hours24 < 12 ? "AM" : "PM"

  text = text.replace(/dddd/g, WEEKDAY_LONG[weekday])
  text = text.replace(/ddd/g, WEEKDAY_SHORT[weekday])
  text = text.replace(/yyyy/g, String(year))
  text = text.replace(/MMMM/g, MONTH_NAMES[month])
  text = text.replace(/MMM/g, MONTH_SHORT[month])
  text = text.replace(/MM/g, pad2(month + 1))
  text = text.replace(/HH/g, pad2(hours24))
  text = text.replace(/mm/g, pad2(minutes))
  text = text.replace(/AP/g, ap)
  text = text.replace(/dd/g, pad2(day))
  text = text.replace(/''yy/g, "'" + pad2(year % 100))
  text = text.replace(/h/g, String(hours12))
  text = text.replace(/\bd\b/g, String(day))
...
if (typeof module !== "undefined") {
  module.exports = {
    ...
    weekdayLongName: weekdayLongName,
    formatClockDateTime: formatClockDateTime
  }
}

...
function outputVolumeName(volume, muted) {
  ...
  if (p === 0) return "Silêncio"
  if (p >= 100) return "Salão de concertos"
  if (p >= 85) return "Modo festa"
  if (p >= 70) return "No talo"
  if (p >= 50) return "Ritmo estável"
  if (p >= 30) return "Som suave"
...
function friendlyDeviceLabel(text) {
  ...
  label = label.replace(/\bMicrophones\b/g, "Microfone")
  return label
}

...
Item {
  ...
  property string reminderBin: Quickshell.env("HOME") + "/.local/bin/omarchy-reminder"
  ...
  readonly property string promptText: root.step === "message" ? "Mensagem do lembrete" : "Lembrar em minutos"
  ...
  function submit() {
    ...
    if (root.step === "minutes") {
      ...
      if (!nextMinutes) {
        Quickshell.execDetached([root.omarchyPath + "/bin/omarchy-notification-send", "Lembrete inválido", "Digite o número de minutos"])
        return
      }
    ...
    if (root.step === "message") {
      var args = [root.reminderBin].concat(ReminderFlowModel.reminderArgs(root.minutes, selection))
      root.dismiss()
